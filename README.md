...
一个面向公开访问场景的心理健康科普 AI 助手，围绕“可信回答、低延迟交互、服务端安全和可部署交付”完整实现。项目基于 DeepSeek 多轮对话、SSE 流式输出和构建期 RAG 知识库，让用户无需注册即可获得带来源引用的科普信息；同时将模型密钥、Prompt 拼接、危机风险提示和知识检索全部收敛在 Vercel Serverless 服务端，适合作为轻量级 AI 应用工程化实践展示。
